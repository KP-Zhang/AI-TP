...
class ConvGRUCell(nn.Module):
    def __init__(self, input_size, input_dim, hidden_dim, kernel_size, bias, dtype):
        ...
        # self.bantch_norm_1 = nn.BatchNorm2d(input_dim + hidden_dim)
        self.relu_gates = nn.LeakyReLU()
        self.conv_gates = nn.Conv2d(in_channels=input_dim + hidden_dim,
                                    out_channels=2 * self.hidden_dim,  # for update_gate,reset_gate respectively
                                    kernel_size=kernel_size,
                                    padding=self.padding,
                                    bias=self.bias)
        # self.relu_can = nn.LeakyReLU()
    ...
    def forward(self, input_tensor, h_cur):
        ...
        combined = self.relu_gates(combined)
        ...
        # combined = self.relu_can(combined)
    ...
    def forward(self, input_tensor, hidden_state=None):
        ...
        for layer_idx in range(self.num_layers):
            ...
            for t in range(seq_len):
                # input current hidden and cell state then compute the next hidden and
